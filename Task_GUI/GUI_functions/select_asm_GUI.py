...
class Example_asm(tk.Frame):
    ...
    def __init__(self, root):
        ...
        for row in range(len(programs)):
            a = row
            tk.Checkbutton(self.scrollFrame.viewPort, text= "Files produced by: " + programs[row], width=45, relief="solid",command=lambda x=a: self.add_remove(str(programs[x]), chosen_programs)).grid(row=row, column=0)    

        tk.Button(self.scrollFrame.viewPort, text="Done, Update Changes", command=lambda x=a: self.printMsg_kill(chosen_programs)).grid(row=row +1, column=0)
...
if __name__ == "__main__":
    ...
    root=tk.Tk()
    root.title('Select Assembly Dependecies')
    Example_asm(root).pack(side="top", fill="both", expand=True)
    root.mainloop()
